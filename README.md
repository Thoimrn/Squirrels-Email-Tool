...
## Overview
The script runs through a series of checks on a raw email, including a header analysis, domain analysis, IP analysis, URL analysis, and a spam word check. All of these are pass/fail checks. When a check is failed, points are added to the overall risk score. Not all failed checks are equal, point weight is determined by overall risk. After the analysis is completed the risk score is summed. A risk score greater than 100 is considered indicitive of a potentially suspicious/malicious/spam email.
